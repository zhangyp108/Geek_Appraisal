...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        ...
        {
            arr_small[count2++] = arr_big[i];	//因为i从0开始，所以要后移一位；*补充注释（说明作用）*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //判断是否为叶子节点*补充判断模块注释*/
        {
            length = huffman[i][3]*findParent(i,huffman,count);   //得到各个叶结点的带权路径长度
            sum += length;/*自行填充代码*/  	//各个叶结点带权路径长度累加得到总长度WPL
        }
    }
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  //判断该节点是否为叶结点，若不为叶结点直接结束循环*补充判断模块注释（说明作用）*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  //若为叶节点，通过逐个对比，找出权重最小的结点*补充判断模块注释（说明作用）*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
...
{
    int length = 0;
    if(huffman[i][0] == -1)     //判断是否有父节点，若已为直接结束；*补充判断模块注释*/
    {
        return 0;
    }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    while(father != -1)
    {
        ...
        {
        	code[start]='1';/*自行填充代码*/		//子结点是父结点的右子树，编码为0
		}
