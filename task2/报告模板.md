...
答：WPL = 56      编码A:0000  B:0001 C:001  D:01  E:1

（2）EEBEACCCCAADACCBCCCCECCCEEECCCE

答：WPL = 56      编码A:001  B:0001  C:1  D:0000  E:01
...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？
   ...
   答：最初是n个中的两个最小的结点形成一个新的结点，随后不断与n个中的结点形成新的结点，所以最后的总数是2n-1个结点

2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？

   答：在遍历字符串时，将代表A的十六进制码改成代表a的，就能读入小写字母；如要同时编码，可以在遍历字符时，加多一个判断条件，若为小写字母，计数。

3. 使用画图工具画出上述两个Huffman生成树的生成过程![avatar](huff1.png)
